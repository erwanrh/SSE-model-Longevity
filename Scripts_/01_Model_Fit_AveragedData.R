...
#This script fit averged data as inputs - Rolling Averaged
# Camarda (2016)
...
for (year in year_min:(year_max-AVG_period)){
  print(paste0('Fitting year : ', year, '...'), quote = FALSE) 

  Data_males <- c()
  Data_females <- c()
  
  for (year2 in seq(year, year+AVG_period)){
  ...
  Data_males <- aggregate(Data_males, by = list(Data_males$x), FUN = mean, na.action = na.omit)
  Data_females <- aggregate(Data_females, by = list(Data_females$x), FUN = mean, na.action = na.omit)
