...
for (year_ in unique(Data_qx_MAR_All$year)[-(1:avg_period)]){
  ...
  try(SSE_males<- morthump(data=Data_males, model='sse' ,lambda.sen = 1), silent = TRUE)
  try(SSE_females<- morthump(data=Data_females, model='sse', lambda.sen = 1), silent = TRUE)
