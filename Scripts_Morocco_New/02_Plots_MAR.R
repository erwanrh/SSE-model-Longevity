...
plot_BSplines <- function(){
  ...
  splines_plot[splines_plot$splinenb %in% 3:22, 'comp']<-'c2'
  splines_plot[splines_plot$splinenb %in% 23:42, 'comp']<-'c3'
...
plot_allyears_senescent <- function(){
  ...
  senescentM <- melt(exp(SSE_males$XX$X2%*%(SSE_coeffcients_males_df[3:22,])),varnames = c('Age','Year'),value.name = 'm')
  senescentM$Comp <- '2'
  senescentM$sex <- 'M'
  
  senescentF <- melt(exp(SSE_females$XX$X2%*%(SSE_coeffcients_females_df[3:22,])),varnames = c('Age','Year'),value.name = 'm')
  senescentF$Comp <- '2'
  senescentF$sex <- 'F'
...
plot_allyears_hump <- function(){
  ...
  humpM <- melt(exp(SSE_males$XX$X3%*%(SSE_coeffcients_males_df[23:42,])),varnames = c('Age','Year'),value.name = 'm')
  humpM$Comp <- '3'
  humpM$sex <- 'M'
  
  humpF <- melt(exp(SSE_females$XX$X3%*%(SSE_coeffcients_females_df[23:42,])),varnames = c('Age','Year'),value.name = 'm')
  humpF$Comp <- '3'
  humpF$sex <- 'F'
...
plot_selectedyears_allComp <- function(Years){
  hump <- melt(exp(SSE_males$XX$X3%*%(SSE_coeffcients_males_df[23:42,])),varnames = c('Age','Year'),value.name = 'm')
  hump$Comp <- '3'
  senescent <- melt(exp(SSE_males$XX$X2%*%(SSE_coeffcients_males_df[3:22,])),varnames = c('Age','Year'),value.name = 'm')
  senescent$Comp <- '2'
...
plot_ExImp <- function() {
  ...
  ex_imp_plot <- ggplot(ex_tot, aes(x= year, y = var, group = sex, color = sex)) +
    ...
    ggtitle(paste0('LE Improvements - ', country_code))
  ex_imp_plot
