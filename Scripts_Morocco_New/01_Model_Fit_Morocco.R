...
for (year_ in unique(Data_qx_MAR_All$year)){
  ...
  try(SSE_males<- morthump(data=Data_males, model='sse'), silent = TRUE) #, x1 = 25, x2 = 40,lambda.sen = 1), silent = TRUE)
  try(SSE_females<- morthump(data=Data_females, model='sse'), silent = TRUE) #,  x1 = 25, x2 = 40, lambda.sen = 1), silent = TRUE)
