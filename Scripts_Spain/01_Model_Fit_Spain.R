...
for (year in seq(2012, 2017)){
  ...
  # Expo
  #Data_males[ Data_males$n==0, c('n', 'm', 'd')] <-0.01
  #Data_females[ Data_females$n==0, c('n', 'm', 'd')] <- 0.01
  ...
  if (!any(is.na(Data_males)) & !any(is.na(Data_females)) ){
    ...
    SSE_females<- morthump(data=Data_females, model='sse')#, lambda.sen = 5, lambda.hump = 5)
