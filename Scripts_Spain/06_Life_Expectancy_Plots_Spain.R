...
ex_m <- data.frame()
ex_f <- data.frame()
for (year_ in as.numeric(colnames(SSE_deathrates_female_df))){
...
ex_m$sex <- 'Male'
ex_m$year <- as.numeric(colnames(SSE_deathrates_female_df))


ex_f$var <- c(NA, diff(ex_f$V1))*12
ex_f$sex <- 'Female'
ex_f$year <- as.numeric(colnames(SSE_deathrates_female_df))
